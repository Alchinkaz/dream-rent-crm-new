...
import { PageProps, ClientItem, ClientDocument, ClientContact } from '../../types';
...
import { RentalsGrid, RentalsTable, carRentals, scootRentals, RentalItem, DateRangePicker, getStatusBadge } from './Rentals';
import { initialCarClients, initialScootClients } from '../../data';
...
export const Clients: React.FC<PageProps> = ({ currentCompany, initialSelectedClientId, onBack }) => {
  ...
  const [clients, setClients] = useState<ClientItem[]>(
    currentCompany.type === 'cars' ? initialCarClients : initialScootClients
  );
  ...
  useEffect(() => {
    const newClients = currentCompany.type === 'cars' ? initialCarClients : initialScootClients;
    setClients(newClients);
    
    // If we have an initial ID (from rentals page navigation)
    if (initialSelectedClientId) {
        const target = newClients.find(c => c.id === initialSelectedClientId);
        if (target) {
  ...
  }, [currentCompany, initialSelectedClientId]);
  ...
  const handleSave = (clientData: ClientItem) => {
    if (editingClient) {
      // Update existing
      setClients(prev => prev.map(c => c.id === clientData.id ? clientData : c));
      
      // Explicitly update selectedClient if we are currently viewing it
      // This ensures the details view reflects changes immediately
...
const ClientDetails: React.FC<{ 
  ...
  onUpdate: (client: ClientItem) => void;
    ...
    const sourceRentals = isCars ? carRentals : scootRentals;
