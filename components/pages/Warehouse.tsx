...
import { carRentals, scootRentals, getStatusBadge as getRentalStatusBadge, RentalsTable, RentalsGrid, DateRangePicker } from './Rentals';
import { initialCarVehicles, initialScootVehicles, VehicleItem } from '../../data';
...
interface Tariff {
  ...
  days: string;
  price: string;
  // Extended fields for the modal logic
  isAllDays?: boolean;
  weekDays?: string[];
}
...
const VehicleDetails: React.FC<VehicleDetailsProps> = ({ vehicle, isCars, onBack, onEdit, onDelete, onUpdate }) => {
    ...
    const allRentals = isCars ? carRentals : scootRentals;
             ...
             <div className="bg-white border-b border-slate-200 px-8 py-6 sticky top-0 z-30">
                <div className="max-w-5xl mx-auto flex items-center justify-between w-full">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={onDelete} className="flex items-center gap-2 px-4 py-2 bg-red-50 border border-red-100 text-red-600 font-medium rounded-lg hover:bg-red-100 transition-colors">
...
export const Warehouse: React.FC<PageProps> = ({ currentCompany, initialSelectedVehicleId, onBack }) => {
  // ... (Existing Warehouse Implementation, no major logic changes needed here) ...
  // Re-pasting the existing logic to ensure file integrity, but the main change was exporting the VehicleForm above.
  
  const [activeTab, setActiveTab] = useState<TabId>('vehicles');
  
  // State for vehicles
  const [carVehicles, setCarVehicles] = useState<VehicleItem[]>(initialCarVehicles);
  const [scootVehicles, setScootVehicles] = useState<VehicleItem[]>(initialScootVehicles);
  ...
  useEffect(() => {
    const newVehicles = currentCompany.type === 'cars' ? initialCarVehicles : initialScootVehicles;
    setCarVehicles(initialCarVehicles);
    setScootVehicles(initialScootVehicles);

    if (initialSelectedVehicleId) {
        const target = newVehicles.find(v => v.id === initialSelectedVehicleId);
        if (target) {
  ...
  }, [currentCompany, initialSelectedVehicleId]);
  ...
  const handleSaveVehicle = (vehicleData: VehicleItem) => {
      const isCars = currentCompany.type === 'cars';
      const setVehicles = isCars ? setCarVehicles : setScootVehicles;

      if (editingVehicle) {
          setVehicles(prev => prev.map(v => v.id === vehicleData.id ? vehicleData : v));
          if (selectedVehicle?.id === vehicleData.id) {
  ...
  const handleUpdateVehicle = (updatedVehicle: VehicleItem) => {
      const isCars = currentCompany.type === 'cars';
      const setVehicles = isCars ? setCarVehicles : setScootVehicles;
  ...
  const currentVehicles = currentCompany.type === 'cars' ? carVehicles : scootVehicles;
  const sourceRentals = currentCompany.type === 'cars' ? carRentals : scootRentals;

  const enrichedVehicles = currentVehicles.map(vehicle => {
      const vehicleRentals = sourceRentals.filter(r => r.vehicle.plate === vehicle.plate);
      vehicleRentals.sort((a, b) => parseRentalDate(b.period.start) - parseRentalDate(a.period.start));
          ...
          <VehicleForm 
            ...
            onCancel={() => {
                if (selectedVehicle && !editingVehicle) {
                     if (selectedVehicle) setActiveView('details');
                     else setActiveView('list');
                } else if (editingVehicle && selectedVehicle) {
                    setActiveView('details');
                } else {
                    setActiveView('list');
                }
